...
class FaceModel(nn.Module):
    def __init__(self, n_shape, n_color, path='model2019_fullHead.h5', device='cuda:0'):
        ...
        self.color_mean = torch.from_numpy(self.Color_mean).unsqueeze(0).to(device) + \
                          (self.param_color@self.Color_basis).reshape(1,-1,3)  
        self.textures = TexturesVertex(verts_features=self.color_mean.to(device))

        #Stockage du modèle de visage
        self.verts = self.verts_mean
    ...
    def reset(self):
        ...
        self.param_color = nn.Parameter(torch.zeros(self.n_color).to(self.device))
        self.param_shape = nn.Parameter(torch.from_numpy(np.ones(self.n_shape,dtype=np.float32)).to(self.device).reshape(-1,1))
        #reset the attributs of the model
        self.verts = self.verts_mean
        self.colors = self.color_mean
        self.mesh = Meshes(verts=self.verts, faces=self.triangles,
                           textures=TexturesVertex(verts_features=self.color_mean.to(self.device)))
